'use strict'
var __awaiter =
  (this && this.__awaiter) ||
  function (thisArg, _arguments, P, generator) {
    function adopt(value) {
      return value instanceof P
        ? value
        : new P(function (resolve) {
            resolve(value)
          })
    }
    return new (P || (P = Promise))(function (resolve, reject) {
      function fulfilled(value) {
        try {
          step(generator.next(value))
        } catch (e) {
          reject(e)
        }
      }
      function rejected(value) {
        try {
          step(generator['throw'](value))
        } catch (e) {
          reject(e)
        }
      }
      function step(result) {
        result.done
          ? resolve(result.value)
          : adopt(result.value).then(fulfilled, rejected)
      }
      step((generator = generator.apply(thisArg, _arguments || [])).next())
    })
  }
var __generator =
  (this && this.__generator) ||
  function (thisArg, body) {
    var _ = {
        label: 0,
        sent: function () {
          if (t[0] & 1) throw t[1]
          return t[1]
        },
        trys: [],
        ops: [],
      },
      f,
      y,
      t,
      g
    return (
      (g = { next: verb(0), throw: verb(1), return: verb(2) }),
      typeof Symbol === 'function' &&
        (g[Symbol.iterator] = function () {
          return this
        }),
      g
    )
    function verb(n) {
      return function (v) {
        return step([n, v])
      }
    }
    function step(op) {
      if (f) throw new TypeError('Generator is already executing.')
      while ((g && ((g = 0), op[0] && (_ = 0)), _))
        try {
          if (
            ((f = 1),
            y &&
              (t =
                op[0] & 2
                  ? y['return']
                  : op[0]
                  ? y['throw'] || ((t = y['return']) && t.call(y), 0)
                  : y.next) &&
              !(t = t.call(y, op[1])).done)
          )
            return t
          if (((y = 0), t)) op = [op[0] & 2, t.value]
          switch (op[0]) {
            case 0:
            case 1:
              t = op
              break
            case 4:
              _.label++
              return { value: op[1], done: false }
            case 5:
              _.label++
              y = op[1]
              op = [0]
              continue
            case 7:
              op = _.ops.pop()
              _.trys.pop()
              continue
            default:
              if (
                !((t = _.trys), (t = t.length > 0 && t[t.length - 1])) &&
                (op[0] === 6 || op[0] === 2)
              ) {
                _ = 0
                continue
              }
              if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) {
                _.label = op[1]
                break
              }
              if (op[0] === 6 && _.label < t[1]) {
                _.label = t[1]
                t = op
                break
              }
              if (t && _.label < t[2]) {
                _.label = t[2]
                _.ops.push(op)
                break
              }
              if (t[2]) _.ops.pop()
              _.trys.pop()
              continue
          }
          op = body.call(thisArg, _)
        } catch (e) {
          op = [6, e]
          y = 0
        } finally {
          f = t = 0
        }
      if (op[0] & 5) throw op[1]
      return { value: op[0] ? op[1] : void 0, done: true }
    }
  }
var __importDefault =
  (this && this.__importDefault) ||
  function (mod) {
    return mod && mod.__esModule ? mod : { default: mod }
  }
Object.defineProperty(exports, '__esModule', { value: true })
exports.cli = exports.BodyParamType = void 0
var prompts_1 = __importDefault(require('prompts'))
var BodyParamType
;(function (BodyParamType) {
  BodyParamType[(BodyParamType['RANDOM_STRING'] = 0)] = 'RANDOM_STRING'
  BodyParamType[(BodyParamType['INCREMENT_VALUE'] = 1)] = 'INCREMENT_VALUE'
})((BodyParamType = exports.BodyParamType || (exports.BodyParamType = {})))
var cli = function () {
  return __awaiter(void 0, void 0, void 0, function () {
    var cmd, baseResponse, addHeader, addBody
    return __generator(this, function (_a) {
      switch (_a.label) {
        case 0:
          cmd = {
            method: '',
            url: '',
            headers: [],
            bodyParams: [],
          }
          return [
            4 /*yield*/,
            (0, prompts_1.default)([
              {
                type: 'text',
                name: 'url',
                message: 'Enter the url you want to request.',
                // initial: 'https://',
              },
              {
                type: 'select',
                name: 'method',
                message: 'Chose the method.',
                choices: [
                  { title: 'GET', description: 'GET method', value: 'GET' },
                  { title: 'POST', description: 'POST method', value: 'POST' },
                  { title: 'PUT', description: 'PUT method', value: 'PUT' },
                ],
                initial: 1,
              },
            ]),
          ]
        case 1:
          baseResponse = _a.sent()
          cmd.url = baseResponse.url
          cmd.method = baseResponse.method
          addHeader = function () {
            return __awaiter(void 0, void 0, void 0, function () {
              var headerCheckResponse, headerResponse
              var _a
              return __generator(this, function (_b) {
                switch (_b.label) {
                  case 0:
                    return [
                      4 /*yield*/,
                      (0, prompts_1.default)({
                        type: 'confirm',
                        name: 'needsHeader',
                        message: 'Do you want to add a header ?',
                        initial: false,
                      }),
                    ]
                  case 1:
                    headerCheckResponse = _b.sent()
                    if (!headerCheckResponse.needsHeader)
                      return [3 /*break*/, 4]
                    return [
                      4 /*yield*/,
                      (0, prompts_1.default)([
                        {
                          type: 'text',
                          name: 'toto',
                          message: 'Name of the header (ex: Authorization)',
                        },
                        {
                          type: 'text',
                          name: 'value',
                          message: 'Value of the header',
                        },
                      ]),
                    ]
                  case 2:
                    headerResponse = _b.sent()
                    ;(_a = cmd.headers) === null || _a === void 0
                      ? void 0
                      : _a.push({
                          name: headerResponse.toto,
                          value: headerResponse.value,
                        })
                    return [4 /*yield*/, addHeader()]
                  case 3:
                    _b.sent()
                    _b.label = 4
                  case 4:
                    return [2 /*return*/]
                }
              })
            })
          }
          return [
            4 /*yield*/,
            addHeader(),
            // Add body section
          ]
        case 2:
          _a.sent()
          addBody = function () {
            return __awaiter(void 0, void 0, void 0, function () {
              var bodyCheckResponse,
                headerResponse,
                bodyParams,
                i,
                dynamicParamResponse
              var _a
              return __generator(this, function (_b) {
                switch (_b.label) {
                  case 0:
                    return [
                      4 /*yield*/,
                      (0, prompts_1.default)({
                        type: 'confirm',
                        name: 'needsBody',
                        message: 'Do you want to add a body to your request ?',
                        initial: false,
                      }),
                    ]
                  case 1:
                    bodyCheckResponse = _b.sent()
                    if (!bodyCheckResponse.needsBody) return [3 /*break*/, 6]
                    return [
                      4 /*yield*/,
                      (0, prompts_1.default)({
                        type: 'text',
                        name: 'body',
                        message:
                          'Copy the body of the request here.\nIf you want dynamic parameters like random strings, integers or increment integer, use the {{myParam}} syntax, they will be configured after.',
                        initial: false,
                      }),
                    ]
                  case 2:
                    headerResponse = _b.sent()
                    cmd.body = headerResponse.body
                    bodyParams = headerResponse.body.match(/{{\w+}}/)
                    i = 0
                    _b.label = 3
                  case 3:
                    if (!(i < bodyParams.length)) return [3 /*break*/, 6]
                    return [
                      4 /*yield*/,
                      (0, prompts_1.default)({
                        type: 'select',
                        name: 'type',
                        message:
                          'What is the type of the dynamic parameter : '.concat(
                            bodyParams[i]
                          ),
                        choices: [
                          {
                            title: 'Increment value',
                            description:
                              'Generates an integer incremented at each call',
                            value: BodyParamType.INCREMENT_VALUE,
                          },
                          {
                            title: 'Random string',
                            description: 'Generates a random string',
                            value: BodyParamType.RANDOM_STRING,
                          },
                        ],
                      }),
                    ]
                  case 4:
                    dynamicParamResponse = _b.sent()
                    ;(_a = cmd.bodyParams) === null || _a === void 0
                      ? void 0
                      : _a.push({
                          name: bodyParams[i],
                          type: dynamicParamResponse.type,
                        })
                    _b.label = 5
                  case 5:
                    i += 1
                    return [3 /*break*/, 3]
                  case 6:
                    return [2 /*return*/]
                }
              })
            })
          }
          return [4 /*yield*/, addBody()]
        case 3:
          _a.sent()
          return [2 /*return*/, cmd]
      }
    })
  })
}
exports.cli = cli
